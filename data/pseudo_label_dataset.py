...
class PseudoLabelDataset(RawDataset):
    def __init__(
        ...
        self.labeler_device = (
            next(pseudo_labeler.parameters()).device
            if labeler_device is None
            else labeler_device
        )
    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            labels = self.pseudo_labeler(raw[None].to(self.labeler_device))[
                0
            ]  # ilastik needs uint input, so normalize afterwards
