...
class ImageCollectionDataset(torch.utils.data.Dataset):
    ...
    def _check_inputs(self, raw_images, label_images, full_check):
        if len(raw_images) != len(label_images):
            raise ValueError(
                f"Expect same number of  and label images, got {len(raw_images)} and {len(label_images)}"
            )
    ...
    def _ensure_patch_shape(
        self, raw, labels, have_raw_channels, have_label_channels, channel_first
    ):
        shape = raw.shape
        ...
        if any(sh < psh for sh, psh in zip(shape, self.patch_shape)):
            if have_raw_channels or have_label_channels:
                raise NotImplementedError(
                    "Padding is not implemented for data with channels"
                )
    ...
    def _get_sample(self, index):
        ...
        raw, label = self._ensure_patch_shape(
            raw, label, have_raw_channels, have_label_channels, channel_first
        )
        shape = raw.shape
