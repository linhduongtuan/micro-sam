...
def get_platynereis_cuticle_dataset(
    path, patch_shape, sample_ids=None, download=False, rois={}, **kwargs
):
    ...
    paths, data_rois = _get_paths_and_rois(
        sample_ids, n_files, os.path.join(cuticle_root, "train_data_%02i.n5"), rois
    )
...
def get_platynereis_cilia_dataset(
    ...
    **kwargs,
    ...
    paths, rois = _get_paths_and_rois(
        sample_ids, n_files, os.path.join(cilia_root, "train_data_cilia_%02i.h5"), rois
    )
    raw_key = "volumes/raw"
...
def get_platynereis_cell_dataset(
    ...
    **kwargs,
    ...
    return torch_em.default_segmentation_dataset(
        data_paths, raw_key, data_paths, label_key, patch_shape, **kwargs
    )
