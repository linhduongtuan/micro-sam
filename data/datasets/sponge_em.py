...
def get_sponge_em_dataset(
    path, mode, patch_shape, sample_ids=None, download=False, **kwargs
):
...
def get_sponge_em_loader(
    path, mode, patch_shape, batch_size, sample_ids=None, download=False, **kwargs
):
    """Dataloader for the segmentation of sponge cells and organelles in EM. See 'get_sponge_em_dataset'."""
    ds_kwargs, loader_kwargs = util.split_kwargs(
        torch_em.default_segmentation_dataset, **kwargs
    )
    ds = get_sponge_em_dataset(
        path, mode, patch_shape, sample_ids=sample_ids, download=download, **ds_kwargs
    )
