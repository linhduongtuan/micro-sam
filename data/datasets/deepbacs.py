...
def _require_deebacs_dataset(path, bac_type, download):
    ...
    if not os.path.exists(zip_path):
        util.download_source(
            zip_path, URLS[bac_type], download, checksum=CHECKSUMS[bac_type]
        )
...
def _get_paths(path, bac_type, split):
    ...
    if bac_type != "mixed":
        raise NotImplementedError(
            f"Currently only the bacteria type 'mixed' is supported, not {bac_type}"
        )
...
def get_deepbacs_dataset(
    ...
    assert (
        bac_type in bac_types
    ), f"{bac_type} is not in expected bacteria types: {bac_types}"
...
def get_deepbacs_loader(
    path, split, patch_shape, batch_size, bac_type="mixed", download=False, **kwargs
):
    """Dataloader for the segmentation of bacteria in light microscopy. See 'get_deepbacs_dataset' for details."""
    ds_kwargs, loader_kwargs = util.split_kwargs(
        torch_em.default_segmentation_dataset, **kwargs
    )
    dataset = get_deepbacs_dataset(
        path, split, patch_shape, bac_type=bac_type, download=download, **ds_kwargs
    )
