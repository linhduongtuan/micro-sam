...
def _preprocess_sem_data(out_path):
    ...
    # get the label paths
    label_folders = glob(os.path.join(data_root, "derivatives", "labels", "sub-rat*"))
    label_folders.sort()
    label_paths = []
    ...
    for i, (rp, lp) in enumerate(zip(raw_paths, label_paths)):
        ...
        with h5py.File(outp, "w") as f:
            ...
            assert (
                len(np.setdiff1d(label_vals, [0, 127, 128, 130, 233, 255])) == 0
            ), f"{label_vals}"
...
def get_axondeepseg_dataset(
    ...
    **kwargs,
    ...
    return torch_em.default_segmentation_dataset(
        all_paths, raw_key, all_paths, label_key, patch_shape, **kwargs
    )
