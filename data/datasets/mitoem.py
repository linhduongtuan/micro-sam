...
def _load_vol(pattern, slice_ids, desc, n_threads, dtype=None):
    ...
    with futures.ThreadPoolExecutor(n_threads) as tp:
        list(
            tqdm(
                tp.map(load_slice, zs, slice_ids[1:]),
                total=len(slice_ids) - 1,
                desc=desc,
            )
        )
...
def _create_volume(out_path, im_folder, label_folder=None, z_start=None):
    ...
    raw = _load_vol(
        os.path.join(im_folder, "im%04i.png"), slices, "load raw", n_threads
    )
    if label_folder is not None:
        labels = _load_vol(
            os.path.join(label_folder, "seg%04i.tif"),
            slices,
            "load labels",
            n_threads,
            dtype="uint64",
    ...
    with z5py.File(out_path, "a") as f:
        f.create_dataset(
            "raw", data=raw, chunks=chunks, compression="gzip", n_threads=n_threads
        )
        if label_folder is not None:
            ds = f.create_dataset(
                "labels",
