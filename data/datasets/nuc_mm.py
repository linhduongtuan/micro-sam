...
def _require_dataset(path, sample):
    ...
    _extract_split(
        os.path.join(input_folder, "Image", "train"),
        os.path.join(input_folder, "Label", "train"),
        os.path.join(sample_folder, "train"),
    )
    _extract_split(
        os.path.join(input_folder, "Image", "val"),
        os.path.join(input_folder, "Label", "val"),
...
def get_nuc_mm_loader(
    path, sample, split, patch_shape, batch_size, download=False, **kwargs
):
