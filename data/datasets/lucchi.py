...
def _load_volume(path, pattern):
    ...
    with futures.ThreadPoolExecutor(n_threads) as tp:
        list(tqdm(tp.map(_loadz, range(1, nz)), desc="Load volume", total=nz - 1))
...
def _create_data(root, inputs, out_path):
    ...
    if labels_argb.ndim == 4:
        ...
        fg_mask = (labels_argb == np.array([255, 255, 255, 255])[None, None, None]).all(
            axis=-1
        )
        labels[fg_mask] = 1
    else:
