...
def _get_image_and_label_paths(root, split, val_fraction):
    ...
    split_file = os.path.join(os.path.split(__file__)[0], f"split_{val_fraction}.json")
...
def get_neurips_cellseg_supervised_dataset(
    ...
    ds = torch_em.data.ImageCollectionDataset(
        image_paths,
        label_paths,
...
def get_neurips_cellseg_unsupervised_dataset(
    ...
    if use_images:
        ...
        datasets.append(
            torch_em.data.RawImageCollectionDataset(
                image_paths,
                patch_shape=patch_shape,
                raw_transform=raw_transform,
                transform=transform,
