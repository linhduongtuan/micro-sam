...
def _generate_binary_masks(
    annot_dict, shape, erose_size=5, obj_size_rem=500, save_indiv=False
):
    ...
    if save_indiv is True:
        mask_edge_indiv = np.zeros((shape[0], shape[1], len(annot_dict)), dtype=np.bool)
        mask_fill_indiv = np.zeros((shape[0], shape[1], len(annot_dict)), dtype=np.bool)
...
def _process_split(root_in, root_out, channels, n_workers, with_labels):
    ...
    with futures.ProcessPoolExecutor(n_workers) as pp:
        list(
            tqdm(
                pp.map(process, inputs, outputs),
                total=len(inputs),
                desc=f"Process data in {root_in}",
            )
        )
...
def _process_hpa_data(path, channels, n_workers, remove):
    ...
    for split in ("train", "test", "valid"):
        ...
        _process_split(
            os.path.join(in_path, split),
            os.path.join(path, out_split),
            channels=channels,
            n_workers=n_workers,
