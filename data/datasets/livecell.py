...
def _create_segmentations_from_annotations(
    annotation_file, image_folder, seg_folder, cell_types
):
    assert (
        COCO is not None
    ...
    for image_id in tqdm(
        image_ids, desc="creating livecell segmentations from coco-style annotations"
    ):
        ...
        if cell_types is not None and (
            not any([cell_type in file_name for cell_type in cell_types])
        ):
            continue
...
def _download_livecell_annotations(path, split, download, cell_types, label_path):
    ...
    seg_folder = (
        os.path.join(path, "annotations", split_name)
    ...
    return _create_segmentations_from_annotations(
        annotation_file, image_folder, seg_folder, cell_types
    )
...
def get_livecell_dataset(
    ...
    **kwargs,
    ...
    image_paths, seg_paths = _download_livecell_annotations(
        path, split, download, cell_types, label_path
    )
