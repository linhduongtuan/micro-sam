...
download_folder = f'./database/EM/{preconfigured_dataset}'
...
# To test the trained model using with EM,
# They also use 4 datasets, named as below!
dataset_names = [
    #"AxonDeepSeg", 
    #"CREMI",
    "Lucchi", 
...
if ds is None:
    ...
    if ds == "AxonDeepSeg":
        ...
        train_loader = torchem_data.get_axondeepseg_loader(download_folder, **kwargs)
        val_loader = torchem_data.get_axondeepseg_loader(download_folder, **kwargs)
    elif ds == "CREMI": # Same as Nucleus_DSB
        ...
        val_loader = torchem_data.get_lucchi_loader(download_folder, split="val", **kwargs)
    elif ds == "MitoEM":
        ...
        train_loader = torchem_data.get_mitoem_loader(download_folder, rois=train_roi, **kwargs)
        val_loader = torchem_data.get_isbi_loader(download_folder, rois=val_roi, **kwargs)
    elif ds == "NucMM-Z":
        train_loader = torchem_data.get_nuc_mm_loader(download_folder, split="train", **kwargs)
        val_loader = torchem_data.get_nuc_mm_loader(download_folder, split="val", **kwargs)
    # monuseg is not fully implemented yet
    # elif ds == "monuseg":
    #     train_loader = torchem_data.get
    elif ds == "PlaytyEM":
        train_roi, val_roi = np.s_[:18, :, :], np.s_[18:, :, :]
        train_loader = torchem_data.get_platynereis_nuclei_loader(download_folder, rois=train_roi, **kwargs)
        val_loader = torchem_data.get_platynereis_nuclei_loader(download_folder, rois=val_roi, **kwargs)

    elif ds == "SNEMI":
        train_roi, val_roi = np.s_[:18, :, :], np.s_[18:, :, :] # Need to check
        train_loader = torchem_data.get_snemi_loader(download_folder, rois=train_roi, **kwargs)
        val_loader = torchem_data.get_snemi_loader(download_folder, rois=val_roi, **kwargs)


    elif ds == "SpongeEM":
        train_roi, val_roi = np.s_[:18, :, :], np.s_[18:, :, :] # Need to check
        train_loader = torchem_data.get_sponge_em_loader(download_folder, rois=train_roi, **kwargs)
        val_loader = torchem_data.get_sponge_em_loader(download_folder, rois=val_roi, **kwargs)
